...
// Depends on: An explicitly configured LitRev-owned feedback endpoint.
...
export const FEEDBACK_CATEGORIES = [
  ...
  { value: "performance", label: "Performance", lead: "LitRev felt slow" },
...
export function formatFeedbackSummary(input: {
  ...
  return [`${lead} in LitRev ${diagnostics.appVersion}${usageContext}.`, "", ...detailLines].join(
    "\n",
  );
}
...
export function resolveFeedbackEndpoint(configuredEndpoint: string | undefined): string {
  ...
  if (!endpoint) {
    throw new Error("Feedback delivery is not configured for this LitRev build.");
  }
  return endpoint;
}

function feedbackEndpoint(): string {
  return resolveFeedbackEndpoint(import.meta.env.VITE_FEEDBACK_ENDPOINT);
}
